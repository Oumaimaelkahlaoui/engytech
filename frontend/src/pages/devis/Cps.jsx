...
export default function CliCpsentForm() {
  ...
  const resetForm = () => { setSent(false); setNomCompelt(''); setEmail(''); setTelephone(''); setTypeProjet(''); setSurface(''); setFiles([]); setTypeStructure(''); setTypeCps(''); }
  const inputStyle = (id) => ({ ...S.input, borderColor: focusedField === id ? "#2a7fa5" : "#dde5f0", boxShadow: focusedField === id ? "0 0 0 3px rgba(42,127,165,0.12)" : "none", background: focusedField === id ? "#fff" : "#f8fafd" })

  async function handleSubmit(e) {
    e.preventDefault(); setSubmitting(true)
    try {
      const { data: devisData, error: devisError } = await supabase.from("devis_types").select("id").eq("nom_devis", "CPS - Métré - estimation").single()
      if (devisError) { console.log("ERREUR TYPE DEVIS:", devisError); return }
      const { data: newDemande, error: insertError } = await supabase.from('demandes_devis').insert([{ devis_type_id: devisData.id, nomcompelt, email, telephone, type_projet: typeProjet, surface, type_structure: typeStructure, type_cps: typeCps }]).select()
  ...
  React.useEffect(() => {
    const id = "cf-styles"; if (document.getElementById(id)) return;
    ...
    el.textContent = `
      ...
      @keyframes cf-pop { from { opacity:0; transform:scale(.7) translateY(12px); } to { opacity:1; transform:scale(1) translateY(0); } }
      @keyframes cf-fadein { from { opacity:0; transform:translateY(16px); } to { opacity:1; transform:translateY(0); } }
      @media (max-width: 900px) { .cf-main { grid-template-columns: 1fr !important; } .cf-sidebar { position: static !important; } }
  ...
  const SuccessPage = () => (
    <div style={{ padding: "clamp(3rem,6vw,5rem) clamp(2rem,4vw,3rem)", display: "flex", flexDirection: "column", alignItems: "center", textAlign: "center", gap: 28 }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 10, maxWidth: "44ch", animation: "cf-fadein .6s .15s both" }}>
        <h3 style={{ fontFamily: "'Playfair Display', Georgia, serif", fontSize: "clamp(1.5rem,3vw,2rem)", fontWeight: 500, color: "#0e1c3a", letterSpacing: "-0.02em", margin: 0, lineHeight: 1.15 }}>Demande envoyée avec{" "}<em style={{ fontStyle: "italic", color: "#2a7fa5" }}>succès</em></h3>
        <p style={{ fontSize: "0.9rem", fontWeight: 300, color: "#7a8faa", lineHeight: 1.8, margin: 0 }}>Notre équipe a bien reçu votre demande et vous contactera sous{" "}<strong style={{ color: "#0e1c3a", fontWeight: 500 }}>24 heures ouvrées</strong>.</p>
      </div>
      <div style={{ display: "flex", gap: 12, flexWrap: "wrap", justifyContent: "center", width: "100%", animation: "cf-fadein .6s .25s both" }}>
        {[{ icon: "👤", label: "Nom", value: nomcompelt }, { icon: "📧", label: "Email", value: email }, { icon: "⏱️", label: "Délai de réponse", value: "Sous 24h ouvrées" }].map(({ icon, label, value }) => (
          <div key={label} style={{ display: "flex", alignItems: "center", gap: 10, padding: "12px 18px", background: "#f8fafd", borderRadius: 10, border: "1px solid #edf1f8", flex: "1 1 180px", textAlign: "left" }}>
            ...
            <div><p style={{ fontSize: "0.6rem", color: "#9aafc4", letterSpacing: "0.1em", textTransform: "uppercase", margin: "0 0 2px" }}>{label}</p><p style={{ fontSize: "0.8125rem", fontWeight: 400, color: "#0e1c3a", margin: 0, wordBreak: "break-word" }}>{value}</p></div>
          </div>
        ))}
      </div>
      ...
      <div style={{ display: "flex", gap: 12, flexWrap: "wrap", justifyContent: "center", animation: "cf-fadein .6s .45s both" }}>
        <button onClick={resetForm} style={{ padding: "12px 22px", borderRadius: 8, border: "1.5px solid #dde5f0", background: "#fff", color: "#7a8faa", fontFamily: "'Outfit',system-ui,sans-serif", fontSize: "0.8rem", fontWeight: 500, cursor: "pointer", letterSpacing: "0.06em" }}
          onMouseEnter={e => { e.currentTarget.style.borderColor = "#2a7fa5"; e.currentTarget.style.color = "#2a7fa5"; }}
          onMouseLeave={e => { e.currentTarget.style.borderColor = "#dde5f0"; e.currentTarget.style.color = "#7a8faa"; }}>← Nouvelle demande</button>
        <a href="/" style={{ padding: "12px 22px", borderRadius: 8, border: "none", background: "linear-gradient(90deg, #1b2f6e, #2a7fa5)", color: "#fff", fontFamily: "'Outfit',system-ui,sans-serif", fontSize: "0.8rem", fontWeight: 500, letterSpacing: "0.08em", textDecoration: "none", display: "inline-flex", alignItems: "center", gap: 8, boxShadow: "0 4px 16px rgba(42,127,165,0.35)" }}>Retour à l'accueil →</a>
      </div>
    </div>
  );
  ...
  const FileZone = () => (
    <label htmlFor="file-upload" style={{ ...S.fileZone, borderColor: fileDrag ? "#2a7fa5" : files.length > 0 ? "#3ab87a" : "#c9d4e8", background: fileDrag ? "rgba(42,127,165,0.04)" : files.length > 0 ? "rgba(58,184,122,0.04)" : "#f8fafd" }}
      onDragOver={(e) => { e.preventDefault(); setFileDrag(true); }} onDragLeave={() => setFileDrag(false)}
      ...
      ) : (<><p style={S.fileText}>Glissez vos fichiers ici ou <span style={{ color: "#2a7fa5", fontWeight: 500 }}>parcourir</span></p><p style={S.fileHint}>PDF, DWG, PNG, JPG — max 20 Mo</p></>)}
  ...
  return (
    <>
      <div style={S.page}>
        <div className="cursor-ring" ref={cursorRef} /><div className="cursor-dot" ref={cursorDotRef} />
        <Navbar />
        <div style={S.hero}>
          <div style={S.heroBg} />
          <div style={S.heroInner}>
            <div>
              ...
              <h1 style={S.heroH1}>CPS & estimation <span style={S.heroItalic}>métrés</span></h1>
              <p style={S.heroSub}>Sélectionnez le type de travaux et envoyez vos plans pour obtenir une estimation rapide et précise.</p>
            </div>
            <div style={S.heroBadge}><span style={{ width: 6, height: 6, borderRadius: "50%", background: "#4ecda4", display: "inline-block" }} />Réponse sous 24h</div>
          </div>
        </div>
        <div className="cf-main" style={S.main}>
          <div style={S.formCard}>
            {sent ? <SuccessPage /> : (
              <>
                <div style={S.formHeader}><div style={S.formHeaderDot} /><h2 style={S.formHeaderTitle}>Informations du projet</h2></div>
                <form onSubmit={handleSubmit} style={S.formBody}>
                  <div className="cf-row" style={S.fieldRow}>
                    <div style={S.fieldCol}><label style={S.label}>Nom complet *</label><input value={nomcompelt} onChange={e => setNomCompelt(e.target.value)} placeholder="Ex : Mohamed Alami" required style={inputStyle("nom")} onFocus={() => setFocusedField("nom")} onBlur={() => setFocusedField(null)} /></div>
                    <div style={S.fieldCol}><label style={S.label}>Adresse email *</label><input value={email} onChange={e => setEmail(e.target.value)} placeholder="exemple@email.com" type="email" required style={inputStyle("email")} onFocus={() => setFocusedField("email")} onBlur={() => setFocusedField(null)} /></div>
                  </div>
                  <div className="cf-row" style={S.fieldRow}>
                    <div style={S.fieldCol}><label style={S.label}>Téléphone *</label><input value={telephone} onChange={e => setTelephone(e.target.value)} placeholder="+212 6 00 00 00 00" required style={inputStyle("tel")} onFocus={() => setFocusedField("tel")} onBlur={() => setFocusedField(null)} /></div>
                    <div style={S.fieldCol}><label style={S.label}>Surface (m²) *</label><input value={surface} onChange={e => setSurface(e.target.value)} placeholder="Ex : 250" type="number" required style={inputStyle("surface")} onFocus={() => setFocusedField("surface")} onBlur={() => setFocusedField(null)} /></div>
                  </div>
                  <div style={S.fieldCol}><label style={S.label}>Type de projet *</label><input value={typeProjet} onChange={e => setTypeProjet(e.target.value)} placeholder="Ex : Villa R+2, Immeuble collectif, Entrepôt..." required style={inputStyle("projet")} onFocus={() => setFocusedField("projet")} onBlur={() => setFocusedField(null)} /></div>

                  {/* Divider CPS */}
                  <div style={S.divider}><div style={S.dividerLine} /><span style={S.dividerText}>Type de CPS</span><div style={S.dividerLine} /></div>
                  <div style={S.fieldCol}>
                    <div style={S.radioGroup}>
                      {CPS_OPTIONS.map((opt) => {
                        const isSelected = typeCps === opt.value;
                        return (
                          <label key={opt.value} style={{ display: "inline-flex", alignItems: "center", gap: 9, padding: "10px 18px", borderRadius: 8, border: `1.5px solid ${isSelected ? "#2a7fa5" : "#dde5f0"}`, background: isSelected ? "rgba(42,127,165,0.07)" : "#f8fafd", cursor: "pointer", fontSize: "0.8125rem", fontWeight: isSelected ? 500 : 300, color: isSelected ? "#2a7fa5" : "#7a8faa", transition: "all 0.2s", userSelect: "none", flexShrink: 0 }}>
                            <span style={{ width: 16, height: 16, borderRadius: "50%", border: `2px solid ${isSelected ? "#2a7fa5" : "#c9d4e8"}`, background: isSelected ? "radial-gradient(circle at center, #2a7fa5 45%, transparent 46%)" : "#fff", flexShrink: 0, display: "inline-block" }} />
                            <input type="radio" name="typeCps" value={opt.value} checked={isSelected} onChange={(e) => setTypeCps(e.target.value)} style={{ display: "none" }} />
                            {opt.label}
                          </label>
                        );
                      })}
                  ...
                  <div style={S.divider}><div style={S.dividerLine} /><span style={S.dividerText}>Documents (optionnel)</span><div style={S.dividerLine} /></div>
                  <div style={S.fieldCol}><FileZone /></div>
                  <button type="submit" style={{ ...S.submitBtn, opacity: submitting ? 0.75 : 1, cursor: submitting ? "not-allowed" : "pointer" }}
                    ...
                    onMouseLeave={(e) => { e.currentTarget.style.filter = ""; e.currentTarget.style.transform = ""; e.currentTarget.style.boxShadow = "0 4px 20px rgba(42,127,165,0.38)"; }} disabled={submitting}>
                    {submitting ? <><span style={{ fontSize: "0.9rem" }}></span>Envoi en cours…</> : <>Envoyer ma demande de devis <span style={{ fontSize: "1rem" }}>→</span></>}
                  </button>
                  <p style={{ fontSize: "0.6875rem", color: "#9aafc4", textAlign: "center", margin: 0, lineHeight: 1.5 }}>Vos informations sont confidentielles et ne seront jamais partagées.</p>
                </form>
              </>
            )}
          </div>
          <div className="cf-sidebar" style={S.sidebar}>
            <div style={S.stepsCard}>
              ...
              <div style={S.stepsBody}>{STEPS.map((s) => (<div key={s.n} style={S.step}><div style={S.stepNum}>{s.n}</div><div style={S.stepText}><p style={S.stepTitle}>{s.title}</p><p style={S.stepDesc}>{s.desc}</p></div></div>))}</div>
            </div>
          </div>
        </div>
        <FloatingMenu /><Footer />
