...
const S = {
  ...
  heroBg: {
    ...
    background: "radial-gradient(ellipse 70% 60% at 65% 50%, rgba(42,127,165,0.22) 0%, transparent 70%),radial-gradient(ellipse 40% 80% at 10% 50%, rgba(58,184,122,0.08) 0%, transparent 65%)",
    zIndex: 0,
  },
  ...
  async function handleSubmit(e) {
    ...
    try {
      const { data: devisData, error: devisError } = await supabase
        ...
        .eq("nom_devis", "Étude VRD (assainissement, voirie, réseaux)").single()
      if (devisError) { console.log("ERREUR TYPE DEVIS:", devisError); return }

      const { data: newDemande, error: insertError } = await supabase
        .from('demandes_devis')
        .insert([{ devis_type_id: devisData.id, nomcompelt, email, telephone, type_projet: typeProjet, surface }])
        .select()
        ...
        <div style={S.hero}>
          ...
          <div style={S.heroInner}>
            <div>
              ...
              <h1 style={S.heroH1}>Étude <span style={S.heroItalic}>VRD</span></h1>
              <p style={S.heroSub}>Étude des Voiries et Réseaux Divers — assainissement, voirie, réseaux d'eau potable, électricité et télécommunication.</p>
            </div>
