...
export default function EfficaciteEnergetique() {
  ...
  const resetForm = () => { setSent(false); setNomCompelt(''); setEmail(''); setTelephone(''); setTypeProjet(''); setSurface(''); setFiles([]); }
  ...
  async function handleSubmit(e) {
    ...
    try {
      const { data: devisData, error: devisError } = await supabase.from("devis_types").select("id").eq("nom_devis", "Étude d’Efficacité Énergétique du Bâtiment").single()
      if (devisError) { console.log("ERREUR TYPE DEVIS:", devisError); return }
      const { data: newDemande, error: insertError } = await supabase.from('demandes_devis').insert([{ devis_type_id: devisData.id, nomcompelt, email, telephone, type_projet: typeProjet, surface }]).select()
        ...
        <div style={S.hero}>
          ...
          <div style={S.heroInner}>
            <div>
              ...
              <h1 style={S.heroH1}>Étude d'<span style={S.heroItalic}>Efficacité Énergétique</span></h1>
              <p style={S.heroSub}>Analyse de la performance énergétique du bâtiment pour réduire la consommation d'énergie et améliorer le confort thermique.</p>
            </div>
            ...
            {sent ? <SuccessPage /> : (
              <>
                ...
                <form onSubmit={handleSubmit} style={S.formBody}>
                  ...
                  <div style={S.divider}><div style={S.dividerLine} /><span style={S.dividerText}>Documents (Plan topo de masse ...)</span><div style={S.dividerLine} /></div>
