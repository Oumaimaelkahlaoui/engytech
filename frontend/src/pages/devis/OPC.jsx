...
export default function OCP() {
  ...
  const resetForm = () => {
    ...
    setTypeProjet(''); setSurface(''); setFiles([]);
    setBudgetProjet(''); setDateDemarrage('');
  }
  ...
  async function handleSubmit(e) {
    ...
    try {
      const { data: devisData, error: devisError } = await supabase
        ...
        .eq("nom_devis", "OPC – Ordonnancement, Pilotage et Coordination").single()
      if (devisError) { console.log("ERREUR TYPE DEVIS:", devisError); return }

      const { data: newDemande, error: insertError } = await supabase
        .from('demandes_devis')
        .insert([{ devis_type_id: devisData.id, nomcompelt, email, telephone, type_projet: typeProjet, surface, budget_projet: budgetProjet, date_demarrage: dateDemarrage }])
        .select()
        ...
        <div style={S.hero}>
          ...
          <div style={S.heroInner}>
            <div>
              ...
              <h1 style={S.heroH1}>Mission <span style={S.heroItalic}>OPC</span></h1>
              <p style={S.heroSub}>Ordonnancement, Pilotage et Coordination des travaux pour assurer le respect des délais, du budget et la bonne organisation du chantier.</p>
            </div>
            ...
            {sent ? <SuccessPage /> : (
              <>
                ...
                <form onSubmit={handleSubmit} style={S.formBody}>
                  ...
                  <div className="cf-row" style={S.fieldRow}>
                    <div style={S.fieldCol}>
                      <label style={S.label}>Budget estimatif du projet TTC *</label>
                      <input value={budgetProjet} onChange={e => setBudgetProjet(e.target.value)} placeholder="Ex : 1 200 000 MAD" required style={inputStyle("budget")} onFocus={() => setFocusedField("budget")} onBlur={() => setFocusedField(null)} />
                    </div>
                    <div style={S.fieldCol}>
                      <label style={S.label}>Date prévue de démarrage *</label>
                      <input type="date" value={dateDemarrage} onChange={e => setDateDemarrage(e.target.value)} required style={inputStyle("date")} onFocus={() => setFocusedField("date")} onBlur={() => setFocusedField(null)} />
                    </div>
                  </div>
